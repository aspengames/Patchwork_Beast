...
func uncorrode():
	if not corroded:
		var tween1 = create_tween()
		#damp /= 1.01
		tween1.tween_property($Corrode.material, "shader_parameter/cutoff_two", -1, 5.0).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT) 
		if $Corrode.material.get("shader_parameter/cutoff_two") <= -1:
			corroded = true
		#else:
